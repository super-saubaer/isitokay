...
var isHTML5 = input.prop("type") === "time";
...
var closeBlock = popoverBody.find(".clockpicker-close-block");
...
var radius;
	  ...
	  // Minutes view
	  let incrementValue = Math.max(options.minutestep, 5);
...
var svg = createSvgElement('svg');
	 ...
	 * @return {string} One of: 'top', 'bottom' or 'viewport-top'.
	 */
...
var elementBottomOffset = element.offset().top + elementHeight;

		
var minVisibleY = elementTopOffset - element[0].getBoundingClientRect().top;

		
var maxVisibleY = minVisibleY + document.documentElement.clientHeight;

		
var isEnoughRoomAbove = elementTopOffset - popoverHeight >= minVisibleY;

		
var isEnoughRoomBelow = elementBottomOffset + popoverHeight <= maxVisibleY;
...
var self = this;
	  ...
	  if (placement === 'top-adaptive' || placement === 'bottom-adaptive') {
		let preferredPlacement = placement.substr(0, placement.indexOf('-'));
	...
	ClockPicker.prototype.parseInputValue = function() {
	  let value = this.input.prop('value') || this.options['default'] || '';
	  ...
	  if (this.options.twelvehour) {
		let period = (value[1] + '').replace(/\d+/g, '').toLowerCase();
...
var outHours = this.hours;

		
var value = ':' + leadingZero(this.minutes);
...
var data = $this.data('clockpicker');
	  ...
	  if (this.length == 1) {
		let returnValue = handleClockPickerRequest.apply(this[0]);
